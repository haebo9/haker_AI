...
class GeminiModel:
    def __init__(self, api_key: str, model_name: str = "gemini-2.0-flash"): # 모델이름을 확인후 수정하세요.
        self.api_key = api_key
        self.model_name = model_name
        genai.configure(api_key=self.api_key)
        self.model = genai.GenerativeModel(self.model_name)
        self.json_data = {}  # JSON 데이터를 저장할 딕셔너리

    def generate_content(self, prompt: str) -> str:
        try:
            # JSON 데이터의 키 값에 해당하는 경우 요약
            if prompt in self.json_data:
                summary_prompt = f"Summarize the following text: {self.json_data[prompt]}"
            ...
            # 새로운 값이 입력된 경우 상관관계 분석
                ...
                if self.json_data:  # 기존 데이터가 있는 경우에만 분석
                    analysis_prompt = f"Analyze the relationship between the following new data: '{prompt}' and the existing data: '{self.json_data}'"
